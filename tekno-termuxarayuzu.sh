...
\033[0m";
	echo ""
	echo -e "\e[1;34m \033[32mLütfen 9 karakterden az girin...\033[0m"
	echo ""
else
	ok=1
fi
...
bash ~/tekno-termuxarayuzu/.banner.sh ${col} ${TNAME}
echo "";
#echo -e '\e[0;35m+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\e[00m';
#echo -e '\033[1;43;30m### SUBSCRIBE MY TELEGRAM CHANNEL ### \033[0m';
#echo -e '\e[0;35m+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+\e[00m';
echo "";
echo -e "
...
tput sgr 0
if [[ ${PROC32} == [Y/y] ]]; then
	if [ -e $HOME/tekno-termuxarayuzu.txt ]; then
		rm $HOME/tekno-termuxarayuzu.txt;
	fi

	if [ -d $HOME/tekno-termuxarayuzu ]; then
	cd $HOME/tekno-termuxarayuzu
	fi
...
clear;
echo -e "\033[31m$(figlet -f teknodroid "tekno-termuxarayuzu")\e[0m"
echo -e "\e[1;32m
+----------------------------------*/
teknodroid : (\e[33m14.4.2021\e[32m)
